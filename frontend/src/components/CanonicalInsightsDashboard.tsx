...
import { matchesLevel2Filter } from '../utils/taxonomy';
...
interface CanonicalInsightsDashboardProps {
  ...
  onProductFilterChange?: (produto: string) => void;
  selectedLevel2Filter?: string;
  onLevel2FilterChange?: (level2: string) => void;
  selectedMeasurementFilter?: string;
  onMeasurementFilterChange?: (measurement: string) => void;
}
...
export const CanonicalInsightsDashboard: React.FC<CanonicalInsightsDashboardProps> = ({
  ...
  onOpenMap,
  onFilterByProduct,
  selectedProductFilter: propProductFilter,
  selectedLevel2Filter: propLevel2Filter,
  selectedMeasurementFilter: propMeasurementFilter,
}) => {
  const [localProductFilter, setLocalProductFilter] = useState('all');
  const [localLevel2Filter, setLocalLevel2Filter] = useState('all');
  const [localMeasurementFilter, setLocalMeasurementFilter] = useState('all');

  const selectedProductFilter = propProductFilter !== undefined ? propProductFilter : localProductFilter;
  const selectedLevel2Filter = propLevel2Filter !== undefined ? propLevel2Filter : localLevel2Filter;
  const selectedMeasurementFilter = propMeasurementFilter !== undefined ? propMeasurementFilter : localMeasurementFilter;

  const filteredArtifacts = useMemo(() => {
    return artifacts.filter(art => {
      if (selectedProductFilter !== 'all' && art.produto !== selectedProductFilter) return false;
      if (selectedLevel2Filter !== 'all' && !matchesLevel2Filter(art, selectedLevel2Filter)) return false;
  ...
  }, [artifacts, selectedProductFilter, selectedLevel2Filter, selectedMeasurementFilter]);
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full min-w-0 max-w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="flat-card min-w-0 p-6 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-neu-card relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="flat-card min-w-0 p-6 md:p-8 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-neu-card space-y-4">
          <h3 className="text-sm font-heading font-bold text-gray-900 dark:text-slate-50 uppercase tracking-wider">
